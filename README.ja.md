...
`wikilinkDirs` の既定は汎用的な出発点（ノート自身のフォルダ＋`notes`・`docs`・`wiki/…`）です。自分のフォルダ構成に合わせるときは **`%APPDATA%\claude-work-desk\user-config.json`** を編集してください。**アプリ同梱の `config.json` ではありません**——そちらは `sync_to_windows.sh` が更新のたびに上書きするので、書いても次のpullで消えます。
...
- WSL越しにはOSの変更通知が来ません（`\\wsl.localhost\` に対する`fs.watch`はそもそも失敗する）。鮮度は**2秒ごとのポーリング**で担保しています＝即時ではなく数秒遅れ、スキャンが重ければ間隔は自動で伸びます。F5か⟳で全体を明示的に更新できます
