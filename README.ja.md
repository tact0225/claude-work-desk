...
*パスを貼るとツリーがそこへ飛びます。`/home/you/project-lane` のようなWSLパスもそのまま使えます。ワークスペースの外にいる間はフォルダ名が青字＋`↗`になり、**`_inbox/`の投入先は本体のまま動きません**——覗いている最中にドロップしても、間違ったレーンに入ることはありません。何度も戻る場所なら、最下段の`＋`でタブにしておけば以後はワンクリックです。*
...
- **差分（開いた時点 → 今）**: プレビューヘッダの**差分**を押すと、そのファイルを**開いた時点から今まで**に変わった行だけが1カラムで出る（削除=赤・追加=緑）。基準は自動更新では動かさない＝エージェントが5回書き換えても、開いた時からの変更が**まとめて1画面**で見える。出し方は**全文**と**変更箇所**の2つから選べる（下記）。何か変わっていればボタン自体に`●`が付き、押さなくても気づける。**確認済み**を押すと基準が今の内容に進み、以後は「確認済みにした時点 → 今」の差分になる。書き換わった範囲が広すぎる時は差分を出さずに「大きすぎます」と告げるが、そこから抜ける道も**確認済み**（基準を今に進めれば次の書き換えからまた差分が出る）。自分が入力モードで保存した変更は基準に取り込まれる（自分の編集はエージェントの修正に混ざらない。未確認の変更が残ったまま入力モードに入ろうとすると一度確認する）。対象はMarkdownとコード、入力モード中は出さない
  - **全文 / 変更箇所**: 差分をどこまで出すかのトグル。**全文**は変わっていない行も全部出す＝上から通して読み、**赤の行だけ拾えば直す前**・**緑の行だけ拾えば直した後**が頭から終わりまで読める（どちらが良くなったかを読み比べて決めるための表示。変更箇所だけだと前後が切れてその判断ができない）。**全文はファイルをそのまま出す**＝frontmatter もコードブロックも画像リンクも空行も落とさない（下の文章モードの絞り込みは掛けない。掛けたまま「全文」と名乗ると、緑だけ拾って読んでも直した後の全文にならないため。したがって全文表示中は文章／コードのトグルは出ない）。無変更行も普通の文字色で出し、`.md` などでは等幅をやめて行間を開ける＝読み物として読める形にする。**変更箇所**は従来どおり、変更の前後3行だけ残して`… N行省略 …`に畳む＝何を触ったかを素早く確かめる表示。**既定は文章モードなら全文・コードモードなら変更箇所**（記事は通して読むもの、ソースは頭から全部読まないもの、という使い方の違いをそのまま既定にしている）。切り替えはそのファイルにだけ効く。**差分に出す行数（無変更＋赤＋緑の合計）が 20,000行 を超える時**は、全文を選んでも変更箇所だけに落とし、落としたことを画面に出す（黙って固まらせない・黙って畳まない）。⚠ 全文は絞り込みを掛けないぶん比べる範囲が広く、**「大きすぎます」に当たることがある**（打ち切りは比べる量で決まるので、全文かどうかで結果が変わる）。その時は**「変更箇所」に切り替えると出ることがある**＝打ち切り画面にその案内を出している（ここで**確認済み**を押すと基準が進んで、読まないまま変更内容を失う）
  - **文章モード / コードモード**: 記事とコードで差分に出したいものが違うので分けてある。**拡張子で自動判定**（`.md` `.markdown` `.txt` は文章モード、それ以外はコードモード）、差分画面の上のトグルでその場で行き来できる（切り替えはそのファイルにだけ効く）。**文章モード**は frontmatter・コードブロック・生HTMLタグ・画像リンク・空行の増減を差分から外し、本文がどう変わったかだけを残す。**コードモード**は何も落とさず全部の行を比べる。落とした側にだけ変更があった時も画面は畳まず「本文以外にも N行 の変更があります」と告げる（`●` の点灯は生の中身で見るので、絞り込みで見落とす経路は作らない）
  - **基準はファイルごとに持つ**＝別のファイルを覗いても消えないし、戻ってきた時に取り直さない。同じファイルを再クリック／ダブルクリックしても動かない。留守の間の書き換えは、戻ってきた時にそのまま差分として読める（溜め込まないよう 20ファイル / 合計4,000,000文字 を超えたら、しばらく見ていないものから落とす）
  - **基準は Desk を閉じても残る**。閉じる・リロードする・表示言語を切り替える、で消えないので、「夜に**確認済み**を押して閉じる → 留守の間に書き換えられる → 翌朝また開く」がそのまま差分として読める。強制終了で閉じた時も残る（ただしディスクへの書き込みはブラウザ側が数秒遅れるうえ、保存は全体を1つのまとまりとして上書きする作りなので、**最後に書き出された時点より後に取った基準は、何本あってもまとめて巻き戻る**。実測: 十分前に取った1本は残り、落ちる直前の数秒で開いた2枚ぶんは2本とも消えた）。保存先はそのPCのローカル領域だけで、**保存するのは開いた時点のファイルの中身の写し**（リポジトリにも `config.json` にも書き出さない）。枠は合計 500,000文字 で、新しい順に入るぶんだけ入る＝これを超える古い基準や、1本でこの枠を超える巨大なファイルの基準は、次に開いた時には残っていない（そのセッション中は今までどおり効く）
- **ダブルクリック**でフォルダをタブで開く（右クリックの**タブで開く**と同じ経路）／ファイルは既定のアプリで開く。エクスプローラーで開きたい時は右クリックの**開く**／**エクスプローラーで表示**
...
- **フォルダタブ**: 最下段にExcelのシートタブ風の横1列。1タブ＝1フォルダで、押すとツリーがそこへ移る。`＋`からは、ワークスペースの隣で見つかったworktreeレーン（開くたびに実測＝撤収したレーンは自然に消える）・今見ているフォルダ・ダイアログで選んだ任意のフォルダを足せる。`~/.claude` のようなツリーには絶対に出ないフォルダへ行けるのはこの経路。**展開していたフォルダ・選んでいたファイル・スクロール位置**をタブごとに覚えるので、戻れば続きから。右クリックで名前の変更（表示名だけ・パスは変えない）／パスをコピー／タブを閉じる。`Ctrl+Tab`・`Ctrl+Shift+Tab`で隣、`Ctrl+1`〜`Ctrl+9`でn枚目。1枚目はワークスペースで閉じられず、`⌂`は常にそこへ戻る。**タブを切り替えてもドロップの投入先は動かない**——タブが動かすのは「見る場所」だけ。行き先が消えたタブは`⚠`を付けて残し、勝手には消さない（WSLが一瞬途切れただけでタブを失わないため）
- **パス欄**: `▾`でさっき行った場所の履歴（直近20件・`↓`キーでも開く）、`⌂`でワークスペースへ復帰、`↑`で1つ上へ。Enter・`↑`・履歴の行は新しいタブを作らず**今のタブのパスを書き換える**（ブラウザのタブと同じ）。タブの切替では履歴に積まない。ファイルのパスを貼った時は親フォルダを開いてその1枚をプレビュー
- **Mermaidの図**: ` ```mermaid ` のフェンスをレンダリング表示で図として描く（ソース表示は今までどおりフェンスのまま）。色とフォントはアプリの配色・フォント設定から作るので、窓の中で浮かない。Mermaid本体は同梱＝CDNを見に行かないのでオフラインでも描ける。構文が壊れている図は、プレビュー全体を巻き添えにせず元のコードブロック（コピーボタン付き）に戻る
...
リポジトリに実体を置いて再配布している第三者コンポーネントが1つあります＝`renderer/vendor/mermaid.min.js`（Mermaid・MIT・© 2014–2022 Knut Sveidqvist）。その中には DOMPurify（Apache-2.0 / MPL-2.0・© Cure53）なども同梱されています。著作権表示とライセンス本文は [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md) にまとめてあります。
