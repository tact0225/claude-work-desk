...
**Claude Codeワークスペースと、デスクトップの「受け渡し窓」。Windows（WSL）とmacOSで動きます。**
...
- **macOS** — `claude-work-desk-x.y.z-mac.dmg` をダウンロードして開き、アプリを**アプリケーション**へドラッグ。初回起動時に「開発元を確認できません」と出ます（理由は同上）。**右クリック → 開く → 開く**、新しめのmacOSでは**システム設定 → プライバシーとセキュリティ → このまま開く**。どちらも1回だけの手順です。
